  ...
  const submitHandler = async(e) => {
    ...
    if(isLogin){
      ...
      try {
        ...
        dispatch(getUser(res?.data?.user));    ////  when i logged in then user set in initial state of redux
        if(res.data.success){
          navigate("/");
  ...
  return (
    <div className='w-screen h-screen flex items-center justify-center'>
      <div className='flex items-center justify-evenly w-[80%]'>
        <div>
          <img className='ml-5' width={"300px"} src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRf_gBfkpXSRRSzQ548l9sNzoEKoHYzNsBRngnKZrQmkaCXhDq6LJtZhwx_UO7oq-ixKBQ&usqp=CAU" alt="twitter-logo" />
        </div>
        <div>
          <div className='my-5'>
